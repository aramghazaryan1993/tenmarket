...
@section('title', 'Отправленные ответ')
...
@if(!isset($CheckBlockUser->type))
  @foreach($IncomingReplMessagePradam as $key => $value)
        ...
        <div class="message  message2">
            <div class="namep name_coment">
                <img src="{{ asset('img/users/'.$value->img) }}" alt="img">
                    ...
                    <a class="newtext" href="{{ asset('UploadFiles/'.$value->files) }}"  download="{{ asset('UploadFiles/'.$value->files) }}">{{ $value->files}}</a>
                    </div>
            </div>
    ...
    @endforeach
      ...
      <form method="POST" enctype="multipart/form-data" action="{{ route('send reply message pradam') }}" class="one_form">
        {{csrf_field()}}
        <div>
          ...
          <input type="hidden" name="to_id" value="{{ $IncomingReplyProductPradam->to_id }}"  required>
        </div>
...
@endif
   ...
   @if(!isset($CheckBlockUser->type))
    ...
    @endforeach
              ...
              <form method="POST" enctype="multipart/form-data" action="{{ route('send reply message kuplyu') }}" class="one_form">
                {{csrf_field()}}
                <div>
                  {{--  <input type="text" name="send_name" placeholder="@if(!isset($HiBlockUser->type))Ваш текст  @elseif($HiBlockUser->type == 1) inq@ qez chyornia qcel @endif">  --}}
                  <textarea name="send_name" placeholder="@if(!isset($HiBlockUser->type))Ваш текст  @elseif($HiBlockUser->type == 1) inq@ qez chyornia qcel @endif"></textarea>
                  ...
                  <input type="hidden" name="from_id" value="{{ Auth::id() }}"  required>
                  <input type="hidden" name="to_id" value="{{ $IncomingReplyProductKuplyu->to_id }}"  required>
                </div>
                  <button type="submit">
                      Отправить
                  </button>
  ...
  @if(isset($IncomingReplyProductTenOrders))
...
@if(!isset($CheckBlockUser->type))
  @foreach($IncomingReplMessageTenOrders as $key => $value)
    ...
    @endforeach
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('send reply message ten orders') }}" class="one_form">
          {{csrf_field()}}
          <div>
            {{--  <input type="text" name="send_name" placeholder="@if(!isset($HiBlockUser->type))Ваш текст  @elseif($HiBlockUser->type == 1) inq@ qez chyornia qcel @endif">  --}}
            <textarea name="send_name" placeholder="@if(!isset($HiBlockUser->type))Ваш текст  @elseif($HiBlockUser->type == 1) inq@ qez chyornia qcel @endif"></textarea>
            ...
            <input type="hidden" name="from_id" value="{{ Auth::id() }}"  required>
            <input type="hidden" name="to_id" value="{{ $IncomingReplyProductTenOrders->to_id }}"  required>
          </div>
            <button type="submit">
                Отправить
            </button>
  ...
  @endif
   ...
   @if(isset($SendReplMessageRequest))
...
@if(!isset($CheckBlockUser->type))
 @foreach($SendReplMessageRequest as $key => $value)
   ...
   @endforeach
       ...
       <form method="POST" enctype="multipart/form-data" action="{{ route('send reply message request') }}" class="one_form">
         {{csrf_field()}}
         <div>
